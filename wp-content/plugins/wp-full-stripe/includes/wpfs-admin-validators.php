...
class MM_WPFS_Admin_DonationFormValidator extends MM_WPFS_Admin_FormValidator {
    ...
    protected function validateMinimumDonationAmount( $bindingResult, $formModel ) {
        ...
        if ( count( $formModel->getDonationAmounts() ) > 0 &&
            ...
            $suggestedAmount = MM_WPFS_Currencies::formatAndEscapeByAdmin( $formModel->getCurrency(), min( $formModel->getDonationAmounts() ), false, true );
            $minimumAmount = MM_WPFS_Currencies::formatAndEscapeByAdmin( $formModel->getCurrency(), $formModel->getMinimumDonationAmount(), false, true );
            $error = sprintf(
    ...
    protected function validateMinimumPaymentAmount( $bindingResult, $formModel ) {
        ...
        if ( $formModel->getPaymentType() === MM_WPFS::PAYMENT_TYPE_LIST_OF_AMOUNTS && $formModel->getAllowCustomPaymentAmount() == 1 ) {
            ...
            if ( count( $formModel->getOnetimeProducts() ) > 0 && $minimumProductAmount <= $formModel->getMinimumPaymentAmount() ) {
                ...
                $productAmount = MM_WPFS_Currencies::formatAndEscapeByAdmin( $formModel->getCurrency(), $minimumProductAmount, false, true );
                $minimumAmount = MM_WPFS_Currencies::formatAndEscapeByAdmin( $formModel->getCurrency(), $formModel->getMinimumPaymentAmount(), false, true );
                $error = sprintf(
