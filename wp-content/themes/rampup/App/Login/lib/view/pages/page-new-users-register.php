<?php
// use Carbon\Carbon;
// do_action('page_existing_users_register__send_form');
include(get_theme_file_path() . "/App/Login/model/include-new-users-register.php");
?>
<?php
/**
 * Template Name: 新規ユーザー登録フォームページ
 * @package WordPress
...
<div class="inner">
  <form method="POST" action="" id="reserveForm" class="formTemplate">
    ...
    <div class="-tac">
      <h2 class="formTemplate__heading">新規ユーザーを作成</h2>
    </div>
    ...
    <?php if (!$class_user->user_status) { ?>
      <div class="formTemplate__inputBox">
        <label class="formTemplate__inputLable" for="user_status">ステータス<span class="-error">*</span></label>
        <div class="formTemplate__inputItem">
          ...
          <?php if ('blank' === $error['user_status']) { ?>
            <p class="error_msg">※ステータスを選択して下さい</p>
          <?php } ?>
        </div>
      </div>
    <?php } else { ?>
    ...
    <!-- <div class="formTemplate__inputBox">
      <label class="formTemplate__inputLable"
             for="reservation_date">面談希望日</label>
      <div class="formTemplate__inputItem">
        <input class="formTemplate__input"
               ...
               id="reservation_date"
               readonly
               value="<?php echo htmlspecialchars($_GET['booking']); ?>">
      </div>
    </div> -->
    ...
    ?>
      ...
      <button class="js__modal__trigger"
              type="button">
        <a href="<? //= $user_get_email['slug'] 
                  ?>"
           target="_blank"
           rel="noopener">
    ...
    <div class="formTemplate__cta">
      <!-- 
      <label class="<?php echo (!current_user_can('administrator')) ? '-hidden' : ''; ?>"
             for="emailCheck">
        <input type="checkbox"
               name="emailCheck"
               id="emailCheck"
               checked>メールを送信する（管理者のみ表示）
        <br><br>
      </label> -->
